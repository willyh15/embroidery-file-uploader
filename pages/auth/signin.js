    ...
    <div className="signin-container">
      ...
      <form onSubmit={handleLogin} className="signin-form">
        ...
        <div className="destination-toggle">
          <span>Go to:</span>
          <button type="button" className={destination === "/" ? "active" : ""} onClick={() => handleDestinationChange("/")}>Uploader</button>
          <button type="button" className={destination === "/admin" ? "active" : ""} onClick={() => handleDestinationChange("/admin")}>Admin</button>
        </div>
      ...
      <style jsx>{`
        ...
        .signin-form {
          background: #1c1c1c; padding: 2rem; border-radius: 8px;
          box-shadow: 0 0 10px rgba(0,0,0,0.5); max-width: 400px;
        }
        label { display: block; margin: 1rem 0 0.5rem; }
        input {
          width: 100%; padding: 0.5rem;
          background: #2c2c2c; border: 1px solid #444; border-radius: 4px; color: white;
        }
        .destination-toggle {
          display: flex; justify-content: space-between;
          background: #222; padding: 0.5rem; border-radius: 6px; margin: 1.5rem 0;
        }
        .destination-toggle button {
          flex: 1; margin: 0 0.25rem; background: #333;
          color: #ccc; border: none; border-radius: 4px; padding: 0.5rem;
        }
        .destination-toggle button.active {
          background: #0070f3; color: white; font-weight: bold;
        }
        .error-banner {
