...
export default async function handler(req, res) {
  ...
  try {
    ...
    const flaskConvertEndpoint = `${process.env.NEXT_PUBLIC_FLASK_BASE_URL || "https://embroideryfiles.duckdns.org"}/convert`;

    const flaskResponse = await fetch(flaskConvertEndpoint, {
      method: "POST",
    ...
    if (!text.trim()) {
      return res.status(502).json({ error: "Empty response from Flask backend" });
    }
    ...
    try {
      ...
      console.error("Failed to parse Flask response:", text);
